...
///
/// @experimental
function CatspeakIRBuilder(filepath=undefined, env = undefined) constructor {
    /// @ignore
    self.functions = [];
    /// @ignore
    ...
    static createCall = function (callee, args, location=undefined) {
        // __createTerm() will do argument validation
		if (env.interface.isCompileTime(callee[$ "name"])) {
			if (array_all(args, __isAllConstant)) {
				var callee_ = env.interface.get(callee.name);
				var _args = variable_clone(args, 0);
				array_map_ext(_args, function(_elm) {
					return _elm.value;
				});
	
				var _value = method_call(callee_, _args);
				return createValue(_value, location);
			}
		}
