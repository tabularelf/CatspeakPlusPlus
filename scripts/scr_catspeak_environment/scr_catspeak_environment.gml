...
function CatspeakForeignInterface() constructor {
    ...
    self.banList = { };
	/// @ignore
	self.databaseCompileTime = { }; // contains functions marked as compile-time
...
/// Returns whether the foreign symbol is a "compile-time function".
    /// If the symbol hasn't been added then this function returns `false`.
    ///
    /// @experimental
    ///
    ...
    ///
    /// @return {Bool}
    static isCompileTime = function (name) {
        if (databaseCompileTime[$ name] ?? false) {
            return true;
        }
        return false;
    };
    ...
    /// @param {Any} value
    ///   The constant value to add.
    static exposeCompileTimeFunction = function () {
        for (var i = 0; i < argument_count; i += 2) {
            var name = argument[i + 0];
    ...
    static exposeDynamicConstant = function () {
        for (var i = 0; i < argument_count; i += 2) {
			...
			func.__internalCatspeak__ = method_get_index(func);
            database[$ name] = func;
    ...
    static exposeFunction = function () {
        for (var i = 0; i < argument_count; i += 2) {
            ...
            func = is_method(func) ? method_get_index(func) : func;
            database[$ name] = method(undefined, func);
			database[$ name].__internalCatspeak__ = func;
        }
    };
	...
	/// Gets all functions exposed and returns them in an array
	static getExposedFunctions = function() {
		var _array = [];
		var _names = struct_get_names(database);
		for(var _i = 0, _len = array_length(_names); _i < _len; ++_i) {
			if (is_method(database[$ _names[_i]]) && (!(databaseDynConst[$ _names[_i]] ?? false))) {
...
function CatspeakEnvironment() constructor {
    ...
    static parse = function (buff, offset=undefined, size=undefined, filepath=undefined) {
        ...
        var builder = new CatspeakIRBuilder(filepath, self);
        var parser = new parserType(lexer, builder, self);
        var moreToParse;
        do {
