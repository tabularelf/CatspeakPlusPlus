...
function CatspeakGMLCompiler(ir, interface=undefined) constructor {
    ...
    /// @param {String} name
    /// @return {Bool}
    static __isCompileTime = function (name) {
        if (__catspeak_is_nullish(interface)) {
            return false;
        }
        return interface.isCompileTime(name);
    }
    ...
    static __compileCall = function (ctx, term) {
		static __executeConstant = function(_elm, _index) {
			return _elm();
		};
    ...
    static __compileSet = function (ctx, term) {
        ...
        if (targetType == CatspeakTerm.INDEX) {
            ...
            var func = target.key.type == CatspeakTerm.VALUE ? 
				__assignLookupIndexHash[term.assignType] : __assignLookupIndex[term.assignType];
            return method({
				...
				hash: target.key.type == CatspeakTerm.VALUE && is_string(target.key.value) ? variable_get_hash(target.key.value) : -1,
                value : value,
            }, func);
    ...
    static __compileIndex = function (ctx, term) {
        ...
        return method({
			...
			hash: is_string(term.key.value) ? variable_get_hash(term.key.value) : -1,
        }, __catspeak_expr_index_get_hash__);
    };
    ...
    static __assignLookupIndexHash = (function () {
        var db = array_create(CatspeakAssign.__SIZE__, undefined);
        db[@ CatspeakAssign.VANILLA] = __catspeak_expr_index_set_hash__;
        db[@ CatspeakAssign.MULTIPLY] = __catspeak_expr_index_set_mult_hash__;
        db[@ CatspeakAssign.DIVIDE] = __catspeak_expr_index_set_div_hash__;
        db[@ CatspeakAssign.SUBTRACT] = __catspeak_expr_index_set_sub_hash__;
        db[@ CatspeakAssign.PLUS] = __catspeak_expr_index_set_plus_hash__;
        return db;
    })();
...
/// @ignore
/// @return {Any}
function __catspeak_expr_catch_finally__() {
    var result;
    try {
        result = eager();
    } catch (exValue) {
...
function __catspeak_expr_call_method__() {
    ...
    with (method_get_self(callee_) ?? collection_) {
		...
		result = is_undefined(args_) ? callee_() : method_call(callee_, args_);
        break;
    }
    __CATSPEAK_END_SELF;
    return result;
...
/// @ignore
/// @return {Any}
function __catspeak_expr_call_method_constant__() {
    // TODO :: this method call stuff is crap, please figure out a better way
    var collection_ = collection();
    var key_ = key();
    var callee_;
    ...
    var args_ = array_length(args) > 0 ? args : undefined;
    var result = undefined;
    // a weird sharp edge here means that `__CATSPEAK_BEGIN_SELF` needs
    // to use `catspeak_get_self`, but the actual with loop needs to use
    // `method_get_self` (see test "get-self-method")
...
/// @ignore
/// @return {Any}
function __catspeak_expr_call_method_constant_some__() {
    // TODO :: this method call stuff is crap, please figure out a better way
    var collection_ = collection();
    var key_ = key();
    var callee_;
	...
	var _valuesDynamic = argsDynamic;
    var args_ = undefined;
	if (array_length(args) > 0)
    { //var args_ = array_map(args, function(f) { return f() });
        var i = 0;
        var values_ = args;
        args_ = variable_clone(values_, 0);
        repeat (array_length(_valuesDynamic)) {
			var index = _valuesDynamic[i];
            // not sure if this is even fast
            // but people will cry if I don't do it
            var value = values_[index];
            args_[@ index] = value();
            i += 1;
        }
    }
...
function __catspeak_expr_call__() {
    ...
    with (method_get_self(callee_) ?? 
        ...
        //var calleeIdx = !is_undefined(callee_[$ "__internalCatspeak__"]) ? callee_.__internalCatspeak__ : method_get_index(callee_);
...
/// @ignore
/// @return {Any}
function __catspeak_expr_call_constant_some__() {
    var callee_ = callee();
    if (!is_method(callee_)) {
        __catspeak_error_v3_got(dbgError, callee_);
    }
...
/// @ignore
/// @return {Any}
function __catspeak_expr_call_constant__() {
    var callee_ = callee();
    if (!is_method(callee_)) {
        __catspeak_error_v3_got(dbgError, callee_);
    }
...
function __catspeak_expr_call_0__() {
    ...
    if (!is_undefined(method_get_self(callee_))) {
        return callee_();
    }
...
/// @ignore
/// @return {Any}
function __catspeak_expr_index_get_hash__() {
    var collection_ = collection();
    if (is_array(collection_)) {
		var key_ = key();
        return collection_[key_];
    } else if (__catspeak_is_withable(collection_)) {
        return struct_get_from_hash(collection_, hash);
    } else {
        __catspeak_error_v3_got(dbgError, collection_);
    }
}
...
/// @ignore
/// @return {Any}
function __catspeak_expr_index_set_hash__() {
    var collection_ = collection();
    var value_ = value();
    if (is_array(collection_)) {
		var key_ = key();
...
/// @ignore
/// @return {Any}
function __catspeak_expr_index_set_mult_hash__() {
    var collection_ = collection();
    var value_ = value();
    if (is_array(collection_)) {
		var key_ = key();
...
/// @ignore
/// @return {Any}
function __catspeak_expr_index_set_div_hash__() {
    var collection_ = collection();
    var key_ = key();
    var value_ = value();
    if (is_array(collection_)) {
...
/// @ignore
/// @return {Any}
function __catspeak_expr_index_set_sub_hash__() {
    var collection_ = collection();
    var key_ = key();
    var value_ = value();
    if (is_array(collection_)) {
...
/// @ignore
/// @return {Any}
function __catspeak_expr_index_set_plus_hash__() {
    var collection_ = collection();
    var key_ = key();
    var value_ = value();
    if (is_array(collection_)) {
...
function __catspeak_expr_params_get__() {
    var key_ = key();
	if (key_ < 0) return undefined;
	if (key_ >= array_length(args)) return undefined;
    return args[key_];
}
