...
This allows Catspeak to instead only update values that are not constant (such as `num`) at runtime. This also means `power(2, 2)` will be aware that both are constants, and will call `power` directly with the arguments as is on runtime.
...
## Who is this for?
Me, myself and I. It might be also interesting for others who are looking at optimisations for their own compiler, or wants a Catspeak version that works with the latest monthly (2024.14.4 at the time of writing), but it is ultimately just for me.
